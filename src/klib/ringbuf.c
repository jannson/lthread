...
void * ringbuf_append_into(ringbuf_t dst, const void *src, size_t count)
{
    //auto enlarger version
    size_t left = ringbuf_bytes_free(dst);
    if(left < count) {
        struct ringbuf_t old, *new_buf;
        memcpy(&old, dst, sizeof(struct ringbuf_t));
        new_buf = ringbuf_new((dst->size-1) * 2 + count);
        ringbuf_copy(new_buf, dst, ringbuf_bytes_used(dst));
        free(old.buf);
        memcpy(dst, new_buf, sizeof(struct ringbuf_t));
        free(new_buf);
        new_buf = NULL;
    }
    return ringbuf_memcpy_into(dst, src, count);
}
